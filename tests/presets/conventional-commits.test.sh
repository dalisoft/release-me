...
test_commit_1_feat_breaking_major_message() {
  ...
  GPG_NO_SIGN=1 bash "release.sh" --plugins=git --quiet --stable
...
test_commit_2_feat_mark_major_message() {
  ...
  bash "release.sh" --plugins=git --quiet --dry-run
  assert_equals "v1.0.0" "$(git tag -l | tail -1)"

  assert_matches "v2.0.0" "$(bash "release.sh" --plugins=git --verbose --dry-run)"

  GPG_NO_SIGN=1 bash "release.sh" --plugins=git --quiet
...
test_commit_3_feat_mark_scope_major_message() {
  ...
  bash "release.sh" --plugins=git --quiet
...
test_commit_5_docs_no_update_message() {
  ...
  bash "release.sh" --plugins=git --verbose --dry-run
  assert_equals "v4.0.0" "$(git tag -l | tail -1)"
