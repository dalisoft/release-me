...
test_commit_0_2_initial_message_no_change() {
  ...
  assert_matches "Your project has no new commits" "$(bash "$ROOT_DIR/release.sh" --plugins=npm,npm-post,git --preset=workspace --workspace)"
}
test_commit_0_3_skip_change() {
  git commit --quiet -m "chore(workspace1): chore commit" --allow-empty --no-gpg-sign

  bash "$ROOT_DIR/release.sh" --plugins=npm,npm-post,git --preset=workspace --workspace --dry-run --pre-release --quiet
  assert_matches "workspace1-v0.0.1" "$(git tag -l)"
...
test_commit_1_0_stable_major_no_message() {
  ...
  bash "$ROOT_DIR/release.sh" --plugins=npm,npm-post,git --preset=workspace --workspace --stable --quiet
...
test_commit_2_feat_mark_major_message() {
  ...
  bash "$ROOT_DIR/release.sh" --plugins=npm,npm-post,git --preset=workspace --workspace --dry-run
  assert_matches "workspace1-v2.0.0" "$(git tag -l)"

  assert_matches "Your project has no new commits" "$(bash "$ROOT_DIR/release.sh" --plugins=npm,npm-post,git --preset=workspace --workspace --dry-run --verbose)"
