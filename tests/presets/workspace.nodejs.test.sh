...
test_commit_0_3_skip_change() {
  git commit -m "chore(workspace1): chore commit" --allow-empty
...
test_commit_1_0_stable_major_no_message() {
  ...
  bash "$ROOT_DIR/release.sh" --plugins=git,npm,npm-post --preset=workspace --workspace --stable --quiet
  assert_matches "workspace1-v1.0.0" "$(git tag -l)"
}
test_commit_1_1_feat_breaking_major_message() {
  git commit -m "feat(workspace1): allow provided config object to extend other configs" -m "BREAKING CHANGE: \`extends\` key in config file is now used for extending other config files" --allow-empty
...
test_commit_1_1_feat_breaking_major_message_skip() {
  git commit -m "feat(workspace2): allow provided config object to extend other configs" -m "BREAKING CHANGE: \`extends\` key in config file is now used for extending other config files" --allow-empty
...
test_commit_2_feat_mark_major_message() {
  ...
  bash "$ROOT_DIR/release.sh" --plugins=git,npm --preset=workspace --workspace --dry-run --quiet
  assert_matches "workspace1-v2.0.0" "$(git tag -l)"
...
test_commit_3_feat_mark_scope_major_message() {
  git commit -m "feat(workspace1)!: send an email to the customer when a product is shipped" --allow-empty
...
test_commit_4_feat_mark_breaking_scope_major_message() {
  git commit -m "chore(workspace1)!: drop support for Node 6" -m "BREAKING CHANGE: use JavaScript features not available in Node 6." --allow-empty
...
test_commit_5_docs_root_no_update_message() {
  git commit -m "docs: correct spelling of CHANGELOG" --allow-empty
...
test_commit_5_docs_workspace_no_update_message() {
  git commit -m "docs(workspace1): correct spelling of CHANGELOG" --allow-empty
...
test_commit_6_feat_scope_message() {
  git commit -m "feat(workspace1): add Polish language" --allow-empty
...
test_commit_7_fix_multi_message() {
  git commit -m "fix(workspace1): prevent racing of requests" -m "Introduce a request id and a reference to latest request. Dismiss incoming responses other than from latest request." -m "Remove timeouts which were used to mitigate the racing issue but are obsolete now" -m "Reviewed-by: Z" -m "Refs: #123" --allow-empty
...
test_commit_8_revert_message() {
  git commit -m "revert(workspace1): let us never again speak of the noodle incident" -m "Refs: 676104e, a215868" --allow-empty
