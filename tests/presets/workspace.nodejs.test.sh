...
test_commit_1_0_stable_major_no_message() {
  git commit -m "fix(workspace1): patch update" --allow-empty --no-gpg-sign
...
test_commit_1_1_feat_breaking_major_message_skip() {
  git commit -m "feat(workspace2): allow provided config object to extend other configs" -m "BREAKING CHANGE: \`extends\` key in config file is now used for extending other config files" --allow-empty --no-gpg-sign
