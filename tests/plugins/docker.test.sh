...
' >>"${REPO_FOLDER}/Dockerfile"
  ...
  _docker() {
    ...
    if [[ -z "${DOCKER_HUB_PAT-}" ]]; then
      ...
      [[ "${FAKE_PARAMS[0]}" == "build" || "${FAKE_PARAMS[0]}" == "push" ]]
    then
      return 0
    else
      exit 1
  ...
  export DOCKER_HUB_PAT="FAKE_TOKEN"
  export HOME="${REPO_FOLDER}"
...
test_plugin_docker_0_1_initial_message_dryrun() {
  ...
  bash "${ROOT_DIR}/release.sh" --plugins=docker --dry-run
}
test_plugin_docker_0_2_initial_message() {
  assert_matches "Docker tag: v0.0.1 and version: v0.0.1" "$(bash "${ROOT_DIR}/release.sh" --plugins=docker --verbose)"
}
test_plugin_docker_no_dockerfile_fail_message() {
  git commit --quiet -m "fix: update commit" --allow-empty --no-gpg-sign

  rm -rf Dockerfile
  assert_matches "Project does not have Dockerfile" "$(bash -eu "${ROOT_DIR}/release.sh" --plugins=docker --verbose)"
