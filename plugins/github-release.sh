...
release() {
  if [ -z "${IS_DRY_RUN}" ] || [ -z "${NEXT_RELEASE_TAG}" ] || [ -z "${NEXT_RELEASE_VERSION}" ] || [ -z "${NEXT_BUILD_VERSION}" ] || [ -z "${CHECKOUT_SHA}" ]; then
    log_verbose "[github-release] Plugin requires a valid release-me pre-processing"
    return 1
  fi
  if [ -z "${RELEASE_BODY_TITLE}" ] || [ -z "${RELEASE_BODY}" ] || [ -z "${PRE_RELEASE_VERSION}" ]; then
    log_verbose "[github-release] Plugin requires a valid release-me release processing"
    return 1
  fi
  ...
  if [ -n "${GITHUB_TOKEN-}" ]; then
    ...
    if ! ${IS_DRY_RUN}; then
      if ! command gh; then
        printf "%s" "GitHub CLI not found your machine"
        return 1
      fi
